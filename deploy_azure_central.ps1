# Deploy to Azure Web App (Central US - Windows)

$RANDOM_ID = Get-Random -Minimum 1000 -Maximum 9999
$RESOURCE_GROUP = "rg-organic-central-" + $RANDOM_ID
$LOCATION = "centralus"
$PLAN_NAME = "plan-organic-central-" + $RANDOM_ID
$APP_NAME = "organic-central-" + $RANDOM_ID
$ZIP_PATH = "deploy.zip"

Write-Host "Starting Azure Deployment (Central US)..."
...
az appservice plan create --name $PLAN_NAME --resource-group $RESOURCE_GROUP --sku F1Number
Start-Sleep -Seconds 10
# Note: F1Number?? No, just F1. 
# Wait, typo in my thought? No, sku F1.
